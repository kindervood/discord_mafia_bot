...
from helpers import parseMessage, userInActiveGame, isDM, Colours
...
class Game:
    ...
    async def launch(self, message):
        await message.channel.send(
            embed=discord.Embed(
                title="Mafia :dagger:",
                description="Welcome to the village, it's normally quite a peaceful place but recently something *a bit sinister* has been happening when everyone's tucked up in bed...\n\nTo join the game message `{0}join`, then `{0}start` when there are at least {1} players. To leave the game at any point message `{0}leave`.".format(
