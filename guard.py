import discord
from helpers import canManageGuild
...
def botManager(command):
    async def guard(bot, message, args):
        if message.author.id == bot.settings["bot"]["owner"] or message.author.id in \
...
def guildManager(command):
    async def guard(bot, message, args):
        ...
        mRole = len([r for r in message.author.roles if
                     r.id in bot.settings[message.guild.id]["manageRoles"]]) > 0
