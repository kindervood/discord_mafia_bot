...
import guard
from helpers import hasPrefix, userInActiveGame, isDM, parseMessage, Colours
...
class GameBot(discord.Client):
        ...
        try:
            ...
            if permissions:
                ...
                await guild.owner.send(
                    "I'm currently missing the following required Guild permissions: {}".format(
                        missing))
    ...
    def checkCategoryPermissions(self, channel):
        if "category" in self.permissions and channel.category:
            def permissionPredicate(permission):
                return hasattr(channel.category.permissions_for(channel.guild.me),
                               permission) and not getattr(
                    channel.category.permissions_for(channel.guild.me), permission)
    ...
    def checkChannelPermissions(self, channel):
        if "channel" in self.permissions:
            def permissionPredicate(permission):
                return hasattr(channel.permissions_for(channel.guild.me),
                               permission) and not getattr(
    ...
    async def logException(self, exception):
        if "logChannel" in self.settings["bot"]:
            ...
            if channel:
                trace = traceback.format_exception(type(exception), exception,
        ...
        try:
            ...
            if guildPrefix and hasPrefix(message, guildPrefix):
                ...
                if not (guildMatched or gameMatched):
                    ...
                    sentInActiveChannel = recognisedGuild and message.channel.id in \
                                          self.settings[message.guild.id]["activeChannels"]
                    sentInMafiaChannel = message.channel.id in self.mafiaChannels

                    if sentInDMWithActiveGame or (
                            recognisedGuild and (sentInActiveChannel or sentInMafiaChannel)):
...
!destroy - destroys a current running game of Mafia
...
!skip - used by villagers to stay quiet''',
    ...
    async def cBotSettings(self, message, args):
        if len(args) > 1:
            ...
            elif option == "removeuser" and len(message.mentions) > 0 and message.mentions[0].id in \
                    self.settings["bot"]["manageUsers"]:
    ...
    async def cBotTestPermissions(self, message, args):
        ...
        needCat = "Yes" if ("category" in self.permissions and "in_category" in self.permissions[
            "category"]) else "No"

        await message.channel.send(
            "**Missing permissions**\nGuild\n```{}```\nCategory\n```{}```\nChannel\n```{}```\nNeeds Category?: `{}`".format(
                guild, category, channel, needCat))
...
Commands during the game:
        ...
        !accuse @user - used by villagers to accuse others of being in the Mafia
        !skip - used by villagers to stay quiet
You can check the rules of mafia here -> https://desktopgames.com.ua/games/895/mafia_rules_rus.pdf''',
            colour=Colours.DARK_BLUE,
        )
        await message.channel.send(embed=embed)
        ...
        if len(args) > 1:
            ...
            if option == "prefix" and len(args) > 2:
                ...
                await message.channel.send(
                    ...
                    self.settings[message.guild.id]["manageUsers"]:
                ...
                await message.channel.send(
                    "Role {0.mention} added to the manager list".format(message.role_mentions[0]))

            elif option == "removerole" and len(message.role_mentions) > 0 and \
                    message.role_mentions[0].id in self.settings[message.guild.id]["manageRoles"]:
                self.settings[message.guild.id]["manageRoles"].remove(message.role_mentions[0].id)
                await message.channel.send("Role {0.mention} removed from the manager list".format(
    ...
    async def cGuildHere(self, message, args):
        channel = message.channel_mentions[0] if (
                    args[0] == "use" and len(message.channel_mentions) > 0) else message.channel

        if channel.id in self.settings[message.guild.id]["activeChannels"]:
            await message.channel.send(
                "{0} is already active in {1.mention}".format(self.name, channel))
        else:
            needsCategory = ("category" in self.permissions and "in_category" in self.permissions[
                "category"])
            ...
            if notInCategory or missingCategory or missingChannel:
                ...
                if missingChannel:
                    summary += "Channel: {}\n".format(
                        ", ".join(["`{}`".format(p) for p in missingChannel]))

                if missingCategory:
                    summary += "Channel Category: {}\n".format(
                        ", ".join(["`{}`".format(p) for p in missingCategory]))
                ...
                await message.channel.send(
                    "{0} now active in {1.mention}".format(self.name, channel))
